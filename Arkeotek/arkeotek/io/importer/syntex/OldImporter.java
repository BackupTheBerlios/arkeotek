 ...
 * @author Czerny Jean
 */
public class OldImporter extends arkeotek.io.importer.AbstractImporter
{
	 ...
	 * @throws IOException 
	 */
	public OldImporter(Ontology owner, String file_path) throws IOException {
	...
	protected void innerTreatment(int i, int j) throws Exception
	{
		...
		if (i == this.parsers.indexOf(this.documents_parser)) {
			...
			this.results.get(i).put(this.current_result[0], new Lemma((String) this.current_result[1]));
			this.owner.link((Lemma) this.results.get(i).get(this.current_result[0]));
			...
			Relation temp_rel = (Relation) this.owner.get(Relation.KEY).get(position);
...
//			if (particles[particles.length - 1].equals("COM") || particles[particles.length - 1].equals("LEG"))
//				System.out.println(parent_key + " ... " +particles[particles.length - 2]);
			...
			if (isProposition(((DocumentPart) element).getValue())) {
				// Looking whether current document is the original "PROPOSITION"
				if (!((DocumentPart) element).getName().contains("ANTECEDENT"))
					isPROP = true;
				if (isPROP)
